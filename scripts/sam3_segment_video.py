...
def preprocess_video(video_path: str, out_dir: str, scale: float = 1.0):
    ...
    if len(existing) == n_frames and n_frames > 0:
        ...
        while True:
            ...
            cv2.imwrite(str(frames_dir / f"{idx:05d}.jpg"), frame, [cv2.IMWRITE_JPEG_QUALITY, 95])
...
def draw_header(img, count, text):
    h, w = img.shape[:2]
    bar = 44
    ...
    cv2.putText(
        out,
        f"SAM 3 | prompt: {text} | objects: {count}",
        (12, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.72,
        (255, 255, 255),
...
def run_sam3(frames_dir: str, ckpt: str, gpu: int, text: str, out_dir: str):
    ...
    out_dir = ensure_dir(out_dir)
    pkl_path = out_dir / "sam3_results.pkl"
    ...
    print(f"[sam3] pkl saved -> {pkl_path}")
    return str(pkl_path)


def render_video(frames_dir: str, result_pkl: str, out_video: str, fps: float, text: str):
    ...
    for idx, fp in enumerate(frames):
        ...
        vis = draw_header(vis, len(masks), text)
        writer.write(vis)
...
def main():
    ...
    parser.add_argument("--sam3-ckpt", required=True)
    parser.add_argument("--sam3-gpu", type=int, default=3)
    parser.add_argument("--text", default="bolt")
    ...
    render_video(
        frames_dir=meta["frames_dir"],
        result_pkl=pkl_path,
        out_video=str(sam3_dir / "sam3_seg_bolt.mp4"),
        fps=meta["fps"],
        text=args.text,
    )

    print("[done]")
    print(f"result: {sam3_dir / 'sam3_seg_bolt.mp4'}")
